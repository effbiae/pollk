...
get   (){ $curl $url >k.zip;}
clone (){ git clone $g;}
rmed  (){ comm -23 <(cd k.edu; echo *|tr -s ' ' '\n'|sort) <(echo README.md LICENSE $(zipinfo -1 k.zip)|tr -s ' ' '\n'|sort);}
gitit (){ (rm=$(rmed); cd k.edu; git pull; git rm $rm; unzip ../k.zip; git add .; git commit -m "$(date -u)"; git push);}
if [ ! -e etag ]; then etag; clone; fi
if new; then set -x; get; gitit; etag; fi
